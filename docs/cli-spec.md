...
| `--backend <choice>` | `convex` \| `supabase` \| `none` | prompt | Backend in `packages/backend` |
...
- **Hard preconditions, checked before anything is generated — dry runs included** (usage errors, exit 2): `--github` conflicts with `--no-git`, `--public` requires `--github`, a git identity (`user.name` + `user.email`) must exist, resolved as the new repository would see it — global/system config; a caller repo's *local* identity doesn't carry into the fresh `git init`, so it doesn't count — and the target must **not already be a git repository**: verify only inits and commits repos it creates, so with a pre-existing `.git` the generated workspace would sit uncommitted while `--push` shipped the old history (and committing into a user's repo uninvited would sweep up state that isn't groot's). The hint prints the manual plant-then-publish path.
...
| Flag | Values | Default | Notes |
| --- | --- | --- | --- |
| `<scaffold>` | `next` \| `sveltekit` \| `tanstack-start` \| `astro` \| `react-router` \| `nuxt` \| `vite` \| `expo` \| `react-native` \| `tauri` \| `electron` \| `elysia` \| `hono` \| `fastify` \| `convex` \| `supabase` | required | Positional: the scaffold to grow |
