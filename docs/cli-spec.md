...
| Flag | Values | Default | Notes |
| --- | --- | --- | --- |
| `<scaffold>` | `next` \| `sveltekit` \| `tanstack-start` \| `astro` \| `react-router` \| `nuxt` \| `vite` \| `expo` \| `react-native` \| `tauri` \| `electron` \| `elysia` \| `hono` \| `fastify` \| `convex` | required | Positional: the scaffold to grow |
