...
- **Hard preconditions, checked before anything is generated — dry runs included** (usage errors, exit 2): `--github` conflicts with `--no-git`, `--public` requires `--github`, a git identity (`user.name` + `user.email`) must exist for fresh targets, resolved as the new repository would see it — global/system config; a caller repo's *local* identity doesn't carry into the fresh `git init`, so it doesn't count — and a `--dir-conflict merge` target that is already a git repository must instead have at least one commit (verify skips its init+commit block there, and the push rides the existing history).
