...
- **Hard preconditions, checked before anything is generated — dry runs included** (usage errors, exit 2): `--github` conflicts with `--no-git`, `--public` requires `--github`, a git identity (`user.name` + `user.email`) must exist — the initial commit is what gets pushed, so the missing-identity downgrade plain init applies becomes an error here — and a `--dir-conflict merge` target that is already a git repository must have at least one commit (verify skips its init+commit block for pre-existing repos).
